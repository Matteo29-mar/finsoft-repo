...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $email = $row["email"];
    // // Crea un array con tutti i titoli e le descrizioni
    // $messages = array();
    // foreach($rows_privati as $row_privati) {
    //   $emailprivato = $row_privati["email"];
    //   $titolo = $row_privati["titolo"];
    //   $descrizione = $row_privati["descrizione"];
    //   $messages[] = "\n<br>Da: " . $emailprivato . "\n<br>Prodotto: " . $titolo . "\n<br> Descrizione: " . $descrizione;
    // }
    // // Invio dell'email contenente tutti i titoli e le descrizioni
    // $message = implode("\n\n", $messages);
